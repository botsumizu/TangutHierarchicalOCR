...
for _, label in image_datasets['train'].samples:
    train_samples_weight.append(train_weights[label].item())  # 使用.item()获取标量值
...
print(f"检测到的类别 ({len(class_names)}类): {class_names}")
...
print(f"模型架构: ResNet18 (最后一层调整为 {len(class_names)} 类输出)")
...
def train_model(model, criterion, optimizer, scheduler, num_epochs):
    ...
    history = {
        ...
        'val_loss': [], 'val_acc': [],
        'learning_rate': [],
        'class_accuracy': {class_name: [] for class_name in class_names}  # 记录每个类别的准确率
    }
