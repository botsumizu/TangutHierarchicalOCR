...
class THOCRSystem:
    ...
    def load_structure_classifier(self):
        try:
            ...
            model.eval()
            print("load structure classifier successfully.")
            return model.to(self.device)
        except Exception as e:
            print(f"failed to load structure classifier.: {e}")
            return None
    ...
    def load_recognizer(self, structure_type):
        try:
            ...
            model.load_state_dict(torch.load(recognizer_model_path,
                                             map_location=self.device))
            model.eval()
            print(f"{structure_type}- recognizer load successfully ({num_classes})")
            return model.to(self.device)
        except Exception as e:
            print(f"failed to load {structure_type}recognizer : {e}")
            return None
    ...
    def predict(self, image_path):
        ...
        for structure_type, recognizer in self.recognizers.items():
            if recognizer is None:
                return {"error": f"failed to load {structure_type}recognizer"}
        ...
        try:
            ...
            return {"error": f"failed to recognize: {e}"}



if __name__ == "__main__":
    print("initialize THOCR...")
    thocr = THOCRSystem()

    print("\nsuccessfully")
    print("character account:", sum(len(chars) for chars in thocr.recognition_classes.values()))
    ...
    if os.path.exists(test_image):
        ...
        if 'error' in result:
            print(f"failed to recognize: {result['error']}")
        else:
            print(f"\nresult:")
            print(f"  structure: {result['structure']}")
            print(f"  character: {result['character']}")
            print(f"  confidence: structure{result['confidence']['structure']:.2%}, "
                  f"character{result['confidence']['character']:.2%}")
    else:
        print(f"\n {test_image} does not exist.")